...
ESMF_IO_INC         = -I$(WRF_SRC_ROOT_DIR)/external/esmf_time_f90 -I$(WRF_SRC_ROOT_DIR)/..
# select -I options for separately installed ESMF library, if present
ESMF_MOD_INC        =  $(ESMF_IO_INC) -I$(WRF_SRC_ROOT_DIR)/..
...
FC              =        $(DM_FC)
CC              =       $(DM_CC) -DMPI2_SUPPORT -DFSEEKO64_OK 
LD              =       echo
RWORDSIZE       =       $(NATIVE_RWORDSIZE)
PROMOTION       =       -i4
ARCH_LOCAL      =       -DNONSTANDARD_SYSTEM_FUNC $(CRESM_WRF_DEFS)
...
CSM_SHR_INCLUDE =      $(SHAREDLIBROOT)/$(SHAREDPATH)/$(COMP_INTERFACE)/$(ESMFDIR)/$(NINST_VALUE)
######################
# POSTAMBLE
...
ARCHFLAGS       =    $(COREDEFS) -DIWORDSIZE=$(IWORDSIZE) -DDWORDSIZE=$(DWORDSIZE) -DRWORDSIZE=$(RWORDSIZE) -DLWORDSIZE=$(LWORDSIZE) \
                     $(ARCH_LOCAL) \
                     $(DA_ARCHFLAGS) \
                      -DDM_PARALLEL -DCCSMCOUPLED -DSEQ_MCT \
                       \
                      -DNETCDF \
                       \
                       \
...
# ESMF_IO_LIB     =    -L$(WRF_SRC_ROOT_DIR)/.. -lesmf_time
...
INCLUDE_MODULES =    $(MODULE_SRCH_FLAG) \
		      ...
		      -I$(CSM_SHR_INCLUDE)/include \
		      -I$(SHAREDLIBROOT)/$(SHAREDPATH)/include \
...
CC_TOOLS_CFLAGS = -DNMM_CORE=$(WRF_NMM_CORE)
 ...
 LIB_EXTERNAL    = \
                      ...
                      -L/software/tamusc/netCDF/4.3.3.1-intel-2015B/lib -lnetcdf -lnetcdff \
                      -L$(WRF_SRC_ROOT_DIR)/external/io_pnetcdf -lwrfio_pnf \
                      -L/software/tamusc/pnetcdf/1.8.1-intel-2015B/lib -lpnetcdf \
                      -L$(CSM_SHR_INCLUDE)/lib -lcsm_chare \
                      -L$(SHAREDLIBROOT)/$(SHAREDPATH)/mct/mct -lmct  
...
NETCDFPATH      =    /software/tamusc/netCDF/4.3.3.1-intel-2015B
NETCDFFPATH     =    /software/tamusc/netCDF-Fortran/4.4.2-intel-2015B
PNETCDFPATH     =    /software/tamusc/pnetcdf/1.8.1-intel-2015B
...
io_only:  esmf_time wrfio_nf wrfio_pnf wrf_ioapi_includes wrfio_grib_share wrfio_grib1 wrfio_int
...
wrfio_grib_share :
	( cd $(WRF_SRC_ROOT_DIR)/external/io_grib_share ; \
          make $(J) CC="$(SCC)" CFLAGS="$(CFLAGS)" RM="$(RM)" RANLIB="$(RANLIB)" CPP="$(CPP)" DEBUG="" \
...
esmf_time :
	( cd $(WRF_SRC_ROOT_DIR)/external/esmf_time_f90 ; \
	cat Makefile | sed -e'/module_symbols_util.o :/d' > make.ccsm ; \
	make $(J) FC="$(SFC) $(PROMOTION) $(FCDEBUG) $(FCBASEOPTS) $(INCLUDE_MODULES)" RANLIB="$(RANLIB)" \
	CPP="$(CPP) -I$(WRF_SRC_ROOT_DIR)/inc -I. $(ARCHFLAGS) $(TRADFLAG)" AR="$(AR)" ARFLAGS="$(ARFLAGS)" \
	SED_FTN="sed -e'/_CALENDARTYPE/d' -e'/_NOCALENDAR/d' -e'/_360DAY/d' -e's/ESMF_Mod/ESMF/' -e's/_CAL_/_CALKIND_/g' -e \"/\!.*'/s/'//g\"" \
	OBJS="module_symbols_util.o module_utility.o" -f make.ccsm )
